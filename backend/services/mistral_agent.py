...
def reconcile_with_erp(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    prompt = (
        ...
        "Your job: given an invoice and the company's open purchase orders, decide the payment status.\n\n"
        "Decisions:\n"
        "  - 'already_paid'  : the invoice matches a PO that is fully paid or a duplicate\n"
        "  - 'not_yet_paid'  : the invoice matches an open or partially-paid PO — payment is due\n"
        "  - 'needs_review'  : no matching PO found, amounts differ significantly, or data is ambiguous\n\n"
        "INVOICE:\n"
        f"{json.dumps(context, ensure_ascii=False, indent=2)}\n\n"
        "OPEN PURCHASE ORDERS FROM ERP:\n"
